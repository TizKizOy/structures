	...
	public:
		...
		void ChangeSaveTableMaterial(DataGridView^ dgv, TextBox^ tx)
		{
			...
			for each (DataGridViewRow ^ row in dgv->Rows) {
				if (!row->IsNewRow) {
					...
					SqlCommand^ command = gcnew SqlCommand("UPDATE SaveTextBoxs SET (Наименование = @value1, Артикул = @value2, [Цена руб.] = @value3, [Ед.изм.] = @value4, Страна = @value5, [Цена $] = @value6, Бренд = @value7 where idusd = @value8", Con);
		...
		void ChangeSaveTableWork(DataGridView^ dgv, TextBox^ tx)
		{
			...
			for each (DataGridViewRow ^ row in dgv->Rows) {
				if (!row->IsNewRow) {
					// Создание команды для вставки данных   
					SqlCommand^ command = gcnew SqlCommand("UPDATE SaveTextBoxs SET (Наименование = @value1, Артикул = @value2, [Цена руб.] = @value3, Тип = @value4, [Ед.изм.] = @value5, Страна = @value6, [Цена $] = @value7 where idusd = @value8", Con);
